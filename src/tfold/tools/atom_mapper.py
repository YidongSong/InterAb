...
from tfold.tools.prot_constants import (
    RESD_MAP_1TO3,
...
class AtomMapper():
        ...
        for frmt_src in self.atom_frmts:
            for frmt_dst in self.atom_frmts:
                if frmt_src == frmt_dst:
                    continue
                self.aidx_map_dict[(frmt_src, frmt_dst)] = self.__build_aidx_map(frmt_src, frmt_dst)
    ...
    def run(self, aa_seq, atom_tns_src, frmt_src=None, frmt_dst=None, method='v2'):
        """Run the atom mapper.
        ...
        * atom_tns_dst: per-atom tensor in the target format of size L x M_o (x *)
        ...
        if atom_tns_src.device != torch.device('cpu'):
            return self.run(
                aa_seq, atom_tns_src.cpu(), frmt_src, frmt_dst, method).to(atom_tns_src.device)

        # choose different atom mapping methods
        assert (frmt_src is not None) and (frmt_dst is not None)
        if method == 'v1':
            atom_tns_dst = self.__run_v1(aa_seq, atom_tns_src, frmt_src, frmt_dst)
        elif method == 'v2':
            atom_tns_dst = self.__run_v2(aa_seq, atom_tns_src, frmt_src, frmt_dst)
        else:
            raise ValueError(f'unrecognized atom mapping method: {method}')
    ...
    def run_batch(self, aa_seqs, atom_tns_src, frmt_src=None, frmt_dst=None, method='v2'):
        ...
        * atom_tns_dst: batched per-atom tensor in the target format of size N x L x M_o (x *)
        ...
        atom_tns_dst = self.atom_mapper.run(
            ''.join(aa_seqs), atom_tns_src.view(-1, *atom_tns_src.shape[2:]),
            frmt_src=frmt_src, frmt_dst=frmt_dst, method=method,
        )  # (N x L) x M_o (x *)
    ...
    def __run_v1(self, aa_seq, atom_tns_src, frmt_src, frmt_dst):
        """Run the atom mapper - v1."""

        # force to use the CPU-based conversion routine
        if atom_tns_src.is_cuda:
            atom_tns_dst = self.__run_v1(aa_seq, atom_tns_src.cpu(), frmt_src, frmt_dst)
            return atom_tns_dst.to(atom_tns_src.device)
        ...
        atom_tns_dst = torch.zeros(
            [n_resds, n_atoms_dst, *atom_tns_src.shape[2:]], dtype=dtype, device=device)

        # re-map atoms between different formats
        for idx_resd, resd_name_1c in enumerate(aa_seq):
            ...
            for idx_atom_dst, atom_name in enumerate(atom_names_dst):
                if atom_name not in atom_names_src:  # skip non-existent atoms
                    continue
                idx_atom_src = atom_names_src.index(atom_name)
                atom_tns_dst[idx_resd, idx_atom_dst] = atom_tns_src[idx_resd, idx_atom_src]

        return atom_tns_dst


    def __run_v2(self, aa_seq, atom_tns_src, frmt_src, frmt_dst):
    ...
    def __build_aidx_map(self, frmt_src, frmt_dst):
