...
Notes:
* For antibody inputs, the heavy chain ID must be 'H' and the light chain ID must be 'L'.
* For nanobody inputs, the heavy chain ID must be 'H'.
"""
...
class AbAssessor():  # pylint: disable=too-few-public-methods
    """Antibody structure assessor."""

    def __init__(self, atom_set='bb', align_set='fr', scheme='chothia'):
        ...
        assert self.scheme in ['imgt', 'chothia'], f'unrecognized renumber scheme: {self.scheme}'
    ...
    def run(self, fas_fpath, pdb_fpath_natv, pdb_fpath_decy):  # pylint: disable=too-many-locals
        ...
        chain_ids = sorted([x[-1] for x in aa_seq_dict])
        ...
        if len(chain_ids) == 1:
            ...
            metrics = {f'{k}-Nano': v for k, v in metrics_raw.items()}
        elif len(chain_ids) == 2:
            assert chain_ids == ['H', 'L']
            metrics = {}
            for chain_id in chain_ids:
                metrics_raw = self.__calc_metrics(
                    aa_seq_dict[chain_id], pdb_fpath_natv, pdb_fpath_decy, chain_id)
                metrics.update({f'{k}-Ab-{chain_id}': v for k, v in metrics_raw.items()})
        else:
            raise ValueError(f'unexpected number of chains in the FASTA file: {fas_fpath}')

        return metrics


    def __calc_metrics(self, aa_seq, pdb_fpath_natv, pdb_fpath_decy, chain_id):
        """Calculate evaluation metrics for a single chain (Ab-H, Ab-L, or Nano)."""
    ...
    def __get_regions(self, aa_seq_all):
        ...
        chn_type = 'H' if details[0][0]['chain_type'] == 'H' else 'L'
