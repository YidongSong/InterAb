...
from tfold.utils import calc_rot_n_tsl
...
class ProtConverter():
    ...
    def cord2fa(self, aa_seq, cord_tns, cmsk_mat):  # pylint: disable=too-many-locals
        ...
        * amsk_mat: torsion angles' validness masks of size L x K
        ...
        angl_tns = torch.stack([
            torch.cos(angl_vec).view(n_resds, N_ANGLS_PER_RESD),
            torch.sin(angl_vec).view(n_resds, N_ANGLS_PER_RESD),
        ], dim=2)  # L x K x 2
    ...
    def fa2cord(self, aa_seq, fram_tns_bb, fmsk_mat_bb, angl_tns, amsk_mat, atom_set='fa'):  # pylint: disable=too-many-arguments
        ...
        * cmsk_mat: 3D coordinates' validness masks of size L x M
        ...
        cord_tns = torch.zeros((n_resds, N_ATOMS_PER_RESD, 3), dtype=torch.float32, device=device)
        ...
        * The number of local frames per residue depends on the <fram_set> argument:
        ...
        if build_sc:
            ...
            fram_tns_sc = torch.cat([
                rot_tns.view(n_resds, N_ANGLS_PER_RESD, 3, 3),
                tsl_mat.view(n_resds, N_ANGLS_PER_RESD, 1, 3),
            ], dim=2)  # L x K x 4 x 3
    ...
    def __fa2cord_fa(self, resd_name, fram_tns_bb, fmsk_mat_bb, angl_tns, amsk_mat):  # pylint: disable=too-many-arguments,too-many-locals
        ...
        * cmsk_mat: 3D coordinates' validness masks of size L' x M
        ...
        atom_names_pad = atom_names_all + ['X'] * (N_ATOMS_PER_RESD - len(atom_names_all))
        atom_infos = ATOM_INFOS_PER_RESD[resd_name]
        angl_infos = ANGL_INFOS_PER_RESD[resd_name]
    ...
    def __get_dihd_atoms(cls, aa_seq, cord_tns, cmsk_mat):  # pylint: disable=too-many-locals
        ...
        * cmsk_tns_dihd: dihedral-angle-atom 3D coordinates' validness masks of size L x K x 4
        ...
        for resd_name in RESD_NAMES_3C:
            # initialization
            atom_names_all = ATOM_NAMES_PER_RESD[resd_name]
            amsk_vec = np.zeros((N_ANGLS_PER_RESD), dtype=np.int8)
            idxs_mat = np.zeros((N_ANGLS_PER_RESD, 4), dtype=np.int64)
        ...
        ).view(n_resds, N_ANGLS_PER_RESD, 4, 3)
        ...
        cmsk_tns_dihd = torch.gather(cmsk_mat, 1, idxs_mat_full).view(n_resds, N_ANGLS_PER_RESD, 4)
    ...
    @classmethod
    def atom14_to_atom37(self, aa_seq, cord14_tns, cmsk14_mat):  # pylint: disable=too-many-locals
