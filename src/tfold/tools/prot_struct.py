...
from tfold.tools.prot_constants import RESD_NAMES_3C
from tfold.tools.prot_constants import RESD_MAP_1TO3
from tfold.tools.prot_constants import ATOM_NAMES_PER_RESD
from tfold.tools.prot_constants import ATOM_INFOS_PER_RESD
from tfold.tools.prot_constants import ANGL_INFOS_PER_RESD
...
class ProtStruct():  # pylint: disable=too-many-instance-attributes
    ...
    @classmethod
    def atom_to_cord(cls, aa_seq, atom_tns, cmsk_mat=None):
        """Get 3D coordinates from 3D Atoms.

        Args:
        * aa_seq: amino-acid sequence
        * atom_tns: per-atom 3D coordinates of size N x 3
        * cmsk_mat(optional): per-atom valid-or-not masks of size L x M

        Returns:
        * cord_tns: protein structure's 3D coordinates of size L x M x 3
        """

        device = atom_tns.device
        dtype = atom_tns.dtype
        if cmsk_mat is None:
            cmsk_mat = cls.get_cmsk_vld(aa_seq, device)
        shape = (*cmsk_mat.shape, 3)
        cord_tns = torch.zeros(shape, device=device, dtype=dtype)
        cord_tns[cmsk_mat.bool()] = atom_tns

        return cord_tns

    @classmethod
    def calc_ppi_sites(cls, prot_data, chn_ids, dist_thres=10):
        """Calculate protein-protein interaction (PPI) for chn_ids."""

        assert len(chn_ids) == 2

        chn_data_pri = prot_data[chn_ids[0]]['base']
        chn_data_sec = prot_data[chn_ids[1]]['base']

        seq_pri = chn_data_pri['seq'].replace('X', 'G')
        seq_sec = chn_data_pri['seq'].replace('X', 'G')

        cord_mat_pri = ProtStruct.get_atoms(seq_pri, chn_data_pri['cord'], ['CA'])
        cmsk_vec_pri = ProtStruct.get_atoms(seq_pri, chn_data_pri['cmsk'], ['CA'])
        cord_mat_sec = ProtStruct.get_atoms(seq_sec, chn_data_sec['cord'], ['CA'])
        cmsk_vec_sec = ProtStruct.get_atoms(seq_sec, chn_data_sec['cmsk'], ['CA'])
    ...
    @classmethod
    def calc_inter_contacts(cls, prot_data, chn_ids, dist_thres=10):
        """Calculate inter-chain contacts for chn_ids."""
