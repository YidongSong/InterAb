"""SE(3)-related utility functions."""
...
def calc_plnr_angl_batch(cord_tns):
    ...
    * cord_tns: 3D coordinates of size N x 3 x 3
    ...
    Returns:
    * rad: planar angles (in radian, ranging from 0 to pi) of size N
    """
    ...
    rad = torch.arccos(torch.clip(torch.sum(a1 * a2, dim=1) / (n1 * n2 + eps), -1.0, 1.0))
...
def kabsch(cord_mat_pri, cord_mat_sec):
    """Solve the optimal rotation matrix for transforming <cord_mat_pri> to <cord_mat_sec>.

    Args:
    * cord_mat_pri: primary 3D coordinates of size N x 3
    * cord_mat_sec: secondary 3D coordinates of size N x 3

    Returns:
    * rota_mat: optimal rotation matrix of size 3 x 3

    Note:
    * Both <cord_mat_pri> and <cord_mat_sec> should be centered (mean vector subtracted).

    Reference:
    * https://en.wikipedia.org/wiki/Kabsch_algorithm
    """

    h_mat = torch.matmul(cord_mat_pri.permute(1, 0), cord_mat_sec)
    u_mat, _, vh_mat = torch.linalg.svd(h_mat)
    d_mat = torch.eye(3, dtype=cord_mat_pri.dtype, device=cord_mat_pri.device)
    d_mat[2, 2] = torch.sign(
        torch.linalg.det(torch.matmul(vh_mat.permute(1, 0), u_mat.permute(1, 0))))
    rota_mat = torch.matmul(vh_mat.permute(1, 0), torch.matmul(d_mat, u_mat.permute(1, 0)))
