...
class MSATransformer(nn.Module):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument(
            ...
            default=2 ** 14,
            type=int,
            help=("Used during inference to batch attention computations in a single ",
        ...
        parser.add_argument(
            ...
            type=str,
            metavar="N",
            help="AttentionType TransformerLayer",
        )

        parser.add_argument("--layers", default=12, type=int, metavar="N")
        parser.add_argument("--embed_positions_msa", default=False, type=bool)
        parser.add_argument("--max_tokens", default=2 ** 20, type=int)
        parser.add_argument("--max_positions", default=1024*3, type=int)

    def __init__(self, args, alphabet = Alphabet.from_architecture('MSA Transformer')):
        super().__init__()
        self.args = args
        self.alphabet_size = len(alphabet)
        self.padding_idx = alphabet.padding_idx
        ...
        if 'AttentionType' not in self.args:
            self.args.AttentionType = 'origin'

        if self.args.AttentionType == 'AxialTransformerLayerEqual2D':
            self.layers = nn.ModuleList(
                [
                    AxialTransformerLayerEqual2D(
                        self.args.embed_dim,
        ...
        elif self.args.AttentionType == 'origin':
            self.layers = nn.ModuleList(
                [
                    AxialTransformerLayer(
                        self.args.embed_dim,
            ...
            print('wrong attention layers', self.args.AttentionType)
    ...
    def forward_layer(self, layer,x,padding_mask,need_head_weights):
    ...
    def forward(
        ...
        if pos_emb:
            x += self.embed_positions(tokens.reshape(batch_size * num_alignments, seqlen)).view(x.size())
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            x = checkpoint(self.forward_layer, layer, x, padding_mask, need_head_weights)
    ...
    def forward_two_crops(
        ...
        assert tokens0.ndim == 3 and  tokens1.ndim == 3
        batch_size, num_alignments, seqlen0 = tokens0.size()
        batch_size, num_alignments, seqlen1 = tokens1.size()

        padding_mask0 = tokens0.eq(self.padding_idx)  # B, R, C

        if not padding_mask0.any():
            padding_mask0 = None

        padding_mask1 = tokens1.eq(self.padding_idx)  # B, R, C

        if not padding_mask1.any():
            padding_mask1 = None

        x0 = self.embed_tokens(tokens0)
        x1 = self.embed_tokens(tokens1)

        # x += self.embed_positions(tokens.view(batch_size * num_alignments, seqlen)).view(x.size())
        x0 += self.embed_positions(tokens0.reshape(batch_size * num_alignments, seqlen0)).view(x0.size())
        x1 += self.embed_positions(tokens1.reshape(batch_size * num_alignments, seqlen1)).view(x1.size())

        if self.msa_position_embedding is not None:
            if x0.size(1) > 1024 or  x1.size(1) > 1024 :
                raise RuntimeError(
                    "Using model with MSA position embedding trained on maximum MSA "
                    f"depth of 1024, but received {x.size(1)} alignments."
                )
            x0 += self.msa_position_embedding[:, :num_alignments]
            x1 += self.msa_position_embedding[:, :num_alignments]

        x0 = self.emb_layer_norm_before(x0)
        x1 = self.emb_layer_norm_before(x1)

        x0 = self.dropout_module(x0)
        x1 = self.dropout_module(x1)

        if padding_mask0 is not None:
            x0 = x0 * (1 - padding_mask0.unsqueeze(-1).type_as(x0))
        if padding_mask1 is not None:
            x1 = x1 * (1 - padding_mask1.unsqueeze(-1).type_as(x1))

        repr_layers = set(repr_layers)

        hidden_representations0 = {}
        hidden_representations1 = {}

        if 0 in repr_layers:
            hidden_representations0[0] = x0
            hidden_representations1[0] = x1

        row_attn_weights = []

        # B x R x C x D -> R x C x B x D
        x0 = x0.permute(1, 2, 0, 3)
        x1 = x1.permute(1, 2, 0, 3)

        for layer_idx, layer in enumerate(self.layers):
            # x0 = layer.forward(x0)
            # x1 = layer.forward(x1)
            x0, x1, row_attn = layer.forward_two_crops(x0, x1, need_head_weights=True)
            # # H x B x L0 x L1 -> B x H x L0 x L1
            row_attn_weights.append(row_attn.permute(1, 0, 2, 3))
            if (layer_idx + 1) in repr_layers:
                hidden_representations0[layer_idx + 1] = x0.permute(2, 0, 1, 3)
                hidden_representations1[layer_idx + 1] = x1.permute(2, 0, 1, 3)

        x0 = self.emb_layer_norm_after(x0)
        x0 = x0.permute(2, 0, 1, 3)  # R x C x B x D -> B x R x C x D

        x1 = self.emb_layer_norm_after(x1)
        x1 = x1.permute(2, 0, 1, 3)  # R x C x B x D -> B x R x C x D

        # last hidden representation should have layer norm applied
        if (layer_idx + 1) in repr_layers:
            hidden_representations0[layer_idx + 1] = x0
        x0 = self.lm_head(x0)

        # last hidden representation should have layer norm applied
        if (layer_idx + 1) in repr_layers:
            hidden_representations1[layer_idx + 1] = x1
        x1 = self.lm_head(x1)

        result = {"logits0": x0,
                  "logits1": x1,
                  "representations0": hidden_representations0,
                  "representations1": hidden_representations1,
                  }

        # row_attentions: B x L x H x L0 x L1
        row_attentions = torch.stack(row_attn_weights, 1)
        result["row_attentions"] = row_attentions
        return result
...
if __name__ == '__main__':
    import argparse
    import tfold2.submodule.esm as esm
    parser = argparse.ArgumentParser()
    MSATransformer.add_args(parser)
    args = parser.parse_args()
    args.layers = 18
    alphabet_msa = esm.Alphabet.from_architecture('MSA Transformer')
    net = MSATransformer(args=args, alphabet=alphabet_msa)

    print(net)
    # B, K, L
    import numpy as np
    x0 = torch.LongTensor(np.zeros([1, 8, 32]))  # L0
    x1 = torch.LongTensor(np.zeros([1, 8, 48]))  # L1

    out = net.forward_two_crops(x0,x1)
    print(out['row_attentions'].shape)
