...
from tfold.third_parties.tfold_released.model import BaseModel
from tfold.third_parties.tfold_released.protein import prot_constants as rc, ProtStruct
    ...
    def output_to_pdb(self, inputs, outputs, filename):
        ...
        for chain_id in chain_ids:
            ...
            if "ptm" in outputs[chain_id]["3d"]["tmsc_dict"]:
                ...
                pred_infos.append(f"REMARK 250 Predicted pTM score: {ptm.item():.4f}")
        ...
        if len(chain_ids) > 1:
            ...
            if "iptm" in outputs[complex_id]["3d"]["tmsc_dict"]:
                ...
                pred_infos.append(f"REMARK 250 Predicted ipTM score: {iptm.item():.4f}")
