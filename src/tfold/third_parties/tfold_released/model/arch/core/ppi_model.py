...
from tfold.third_parties.tfold_released.config import configurable
from tfold.third_parties.tfold_released.protein.data_transform import get_asym_ids, get_entity_ids
from tfold.third_parties.tfold_released.protein.utils import get_mlm_masks, apply_mlm_masks
        ...
        * <asym>, <enty>, and <mask> can be generated w/ utility functions in utils.py.
        ...
        if rtn_sp_feats:
            ...
            for i in range(len(inputs)):
                if seq_len < len(''.join(inputs[i]['seqs'])):
                    seq_len = len(''.join(inputs[i]['seqs']))

            # seq_len = max[len(''.join(inputs[i]['seqs'])) for i in range(len(inputs))]
