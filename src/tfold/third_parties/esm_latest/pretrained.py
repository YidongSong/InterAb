...
import torch
import tfold.third_parties.esm_latest as esm_latest
...
def load_model_and_alphabet_hub(model_name, local_dir = None):

    if local_dir is not None:
        model = f'{local_dir}/{model_name}.pt'
        print(f'load from {model}')
    ...
    if _has_regression_weights(model_name):
        ...
        if local_dir is not None:
            model = f'{local_dir}/{model_name}-contact-regression.pt'
            print(f'load from {model}')
            regression_data = torch.load(model)
        else:
...
def load_model_and_alphabet_core(model_data, regression_data=None):
    ...
    alphabet = esm_latest.Alphabet.from_architecture(model_data["args"].arch)
    ...
    if model_data["args"].arch == "roberta_large":
        ...
        model_type = esm_latest.ProteinBertModel
        ...
        model_type = esm_latest.MSATransformer
...
def esm1b_t33_650M_UR50S(local_dir = None):
    ...
    Returns a tuple of (Model, Alphabet).
    """
    return load_model_and_alphabet_hub("esm1b_t33_650M_UR50S", local_dir=local_dir)
