...
class AlphaFold(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.training:
            num_iters = np.random.randint(1, num_iters)
        ...
        # Run auxiliary heads
        outputs.update(self.aux_heads(outputs, self.disable))

        return outputs


class AF2InitNet(nn.Module):
    """
    Alphafold 2.
    ...
    def __init__(self, config):
        ...
        Args:
            config:
                A dict-like config object (like the one in config.py)
        """
        super(AF2InitNet, self).__init__()
    ...
    def embed_templates(self, batch, z, pair_mask, templ_dim):
        ...
        for i in range(n_templ):
            ...
            if self.config.template.embed_angles:
                ...
                if torch.isnan(torch.mean(a)):
                    raise NotImplementedError
                if torch.isinf(torch.mean(a)):
                    raise NotImplementedError
            ...
            if torch.isnan(torch.mean(t)):
                raise NotImplementedError
            if torch.isinf(torch.mean(t)):
                raise NotImplementedError
    ...
    def iteration(self, feats, m_1_prev, z_prev, x_prev, _recycle=True):
        ...
        if self.config.template.enabled:
            ...
            if(torch.any(template_mask)):
                ...
                if torch.isnan(torch.mean(template_embeds["template_pair_embedding"])):
                    raise NotImplementedError
                if torch.isinf(torch.mean(template_embeds["template_pair_embedding"])):
                    raise NotImplementedError
        ...
        if torch.isnan(torch.mean(outputs['msa'])):
            raise NotImplementedError
        if torch.isinf(torch.mean(outputs['msa'])):
            raise NotImplementedError

        if torch.isnan(torch.mean(outputs['pair'])):
            raise NotImplementedError
        if torch.isinf(torch.mean(outputs['pair'])):
            raise NotImplementedError
    ...
    def forward(self, batch):
        ...
        #TODO force no iteration
        num_iters = 1

        if self.training and num_iters > 1:
            num_iters = np.random.randint(1, num_iters)
