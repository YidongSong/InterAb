...
from openfold.model.outer_product_mean import OuterProductMean, OuterProductMeanPrecision
...
class EvoformerBlock(nn.Module):
    ...
    def forward(
        ...
        if torch.isinf(torch.mean(z)):
            raise NotImplementedError
        if torch.isnan(torch.mean(m)):
            raise NotImplementedError
        if torch.isinf(torch.mean(m)):
            raise NotImplementedError
