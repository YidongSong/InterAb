...
class RcEmbedNet2D(nn.Module):
    """The recycled embedding network for sequence features and pair features."""

    def __init__(
            self,
            n_dims_sfea=384,  # number of dimensions in sequence features
            n_dims_pfea=256,  # number of dimensions in pair features
    ):
        """Constructor function."""
        ...
        self.proj_s = nn.Sequential(
            ...
            nn.Linear(self.n_dims_sfea, self.n_dims_sfea, bias=False),
        )
        self.proj_p = nn.Sequential(
            nn.LayerNorm(self.n_dims_pfea),
            nn.Linear(self.n_dims_pfea, self.n_dims_pfea, bias=False),
        )

    def forward(self, sfea_tns, pfea_tns, rc_inputs=None):
        """Perform the forward pass.

        Args:
        * sfea_tns: seq features of size N x L x D_s
        * pfea_tns: pair features of size N x L x L x D_p
        * rc_inputs: (optional) dict of additional inputs for recycling embeddings
          > sfea: single features of size N x L x D_s
          > pfea: pair features of size N x L x L x D_p

        Returns:
        * sfea_tns: updated MSA features of size N x L x D_s
        * pfea_tns: updated pair features of size N x L x L x D_p
        """
        ...
        if rc_inputs is None:
            rc_inputs = {
                'sfea': torch.zeros_like(sfea_tns),
                'pfea': torch.zeros_like(pfea_tns),
            }

        # calculate update terms for single features
        sfea_tns_rc = self.proj_s(rc_inputs['sfea'])

        # calculate update terms for pair features
        pfea_tns_rc = self.proj_p(rc_inputs['pfea'])

        return sfea_tns_rc, pfea_tns_rc
