...
from tfold.third_parties import esm
from tfold.third_parties import esm_latest
from tfold.third_parties.esm.modules import RobertaLMHead
from tfold.third_parties.esm.modules import init_bert_params
from tfold.modules.xfold.utils import exists
from tfold.modules.xfold.utils import load_pretrain
from tfold.modules.xfold.network.embeddings import MSA_emb
from tfold.modules.xfold.network.embeddings import Pair_emb
from tfold.modules.xfold.network.embeddings import Pair_emb_af2
from tfold.modules.xfold.network.modules import IterativeFeatureExtractor2D
from tfold.modules.xfold.network.modules import MSAExtraStack
from tfold.modules.xfold.network.da_predictor import PairPredictor
from tfold.modules.xfold.network.utils import init_zero_mlp
...
class XFold2D(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Pair embedding
        if config.templ.af2:
            self.pair_emb = Pair_emb_af2(d_msa = self.d_msa,
                                         d_pair = self.d_pair,
                                         p_drop = config.templ.af2_p_drop,
                                         )
        else:
        ...
        # Evoformer-like MSA-Pair recycling
        ...
        # inter-residue distance & orientation predictions
        ...
        self.TokenDropoutColumnwise = DropoutColumnwise(r=config.dropout.TokenDropoutColumnwise)
        self.TokenDropoutRowwise = DropoutRowwise(r=config.dropout.TokenDropoutRowwise)
        ...
        if exists(config.msa_bert.msa_bert_config) and not config.modules.msa_emb.is_onehot:
    ...
    def forward_fea_init(self, tokens, t1ds = None, t2ds = None, is_BKL = True,
        ...
        if exists(self.msa_bert_config) and not config.modules.msa_emb.is_onehot:
        ...
        init_msa = msa_fea
        msa_fea = self.TokenDropoutColumnwise(msa_fea)
        msa_fea = self.TokenDropoutRowwise(msa_fea)

        if config.templ.af2:
            pair_fea, msa_fea = self.pair_emb(msa_fea, t1ds, t2ds, pair = None)
        else:
    ...
    def forward(self, tokens, t1ds = None, t2ds = None, masked_tokens = None, is_fea = False, is_BKL = True,
        ...
        msa_fea, pair_fea, init_msa_fea = self.forward_fea(tokens = tokens,
                                                         ...
                                                         is_BKL = is_BKL,
                                                         n_recycle = n_recycle,
                                                         # tokens_feats = tokens_feats,
                                                         # extra_tokens_feats = extra_tokens_feats,
                                                          )
