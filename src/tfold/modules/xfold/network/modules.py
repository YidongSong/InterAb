...
from tfold.modules.xfold.network.attention import *
from tfold.modules.xfold.network.conv_blocks import SEBottleneck
from tfold.modules.xfold.network.interact import CoevolExtractor
...
class Pair2Pair(nn.Module):
    ...
    def forward(self, x, use_checkpoint=False):
        ...
        for layer in self.encoder:
            x = layer(x) if not use_checkpoint else checkpoint(layer, x)
...
class IterBlock(nn.Module):
    ...
    def forward(self, msa, pair):
        ...
        if self.use_checkpoint:
            msa = self.msa2msa(msa, use_checkpoint=True)
            pair = checkpoint(self.msa2pair, msa, pair)
            pair = self.pair2pair(pair, use_checkpoint=True)
            msa = checkpoint(self.pair2msa, pair, msa)
        else:
...
class IterativeFeatureExtractor2D(nn.Module):
    ...
    def forward(self, msa, pair):
        ...
        # initial updates on MSA & pair features
        msa = self.initial_msa(msa, use_checkpoint=self.use_checkpoint)
        pair = self.initial_pair(pair, use_checkpoint=self.use_checkpoint)

        # iterative updates on MSA & pair features
