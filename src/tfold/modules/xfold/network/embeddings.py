...
from tfold.third_parties.esm.modules import LearnedPositionalEmbedding
from tfold.modules.xfold.network.attention import LayerNorm
from tfold.modules.xfold.network.conv_blocks import SEBottleneck
...
class Pair_emb_af2(nn.Module):
    def __init__(self, d_pair = 256, d_msa = 384, p_drop = 0.0):
        super(Pair_emb_af2, self).__init__()

        self.d_msa = d_msa
        self.d_pair = d_pair

        self.template_angle_embedder = TemplateAngleEmbedder(c_in = 57, c_out = d_msa)

        self.template_pair_embedder = TemplatePairEmbedder(c_in = 88, c_out = 64)

        self.template_pair_stack = TemplatePairStack(
            c_t = 64,
            c_hidden_tri_att = 16,   #     "c_hidden_tri_att": 16,
            c_hidden_tri_mul = 64,   #     "c_hidden_tri_mul": 64,
            no_blocks = 2,           #     "no_blocks": 2,
            no_heads = 4,            #     "no_heads": 4,
            pair_transition_n = 2,   #     "pair_transition_n": 2,
            dropout_rate = p_drop,   #     "dropout_rate": 0.25,
            blocks_per_ckpt = None,  #     "blocks_per_ckpt": blocks_per_ckpt,
            inf=1e9,                 #     "inf": 1e9,
        )
    ...
    def forward(self, msa, t1ds, t2ds, pair = None):
        # [*, S_t, N, C_m]
        angle = self.template_angle_embedder(t1ds)
        # [*, S_t, N, N, C_t]
        t = self.template_pair_embedder(t2ds)
        # [*, N, N, C_z]

        pair_mask = t.new_ones(t.shape[0], t.shape[1], t.shape[2],t.shape[2])
        ...
        if pair is None:
            pair = t.new_zeros(t.shape[0], t.shape[2],t.shape[2], self.d_pair)

        pair = self.template_pointwise_att(
            t,
            pair,
            chunk_size = None,
            template_mask = None,
