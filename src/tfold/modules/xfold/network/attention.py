...
from tfold.modules.xfold.utils import exists, default
from tfold.modules.xfold.network.activation import get_activation_fn
from tfold.modules.xfold.network.rpe import RotaryEmbedding, apply_rotary_pos_emb, RPE_T5
...
from tfold.modules.common import Attention
'''
...
class AxialAttention(nn.Module):
    ...
    def forward(self, x, edges = None, mask = None):
        ...
        if exists(self.edges_to_attn_bias) and exists(edges):
            ...
            #attn_bias = repeat(attn_bias, 'b h i j -> (b x) h i j', x = axial_dim)  # no need to repeat
...
class PairwiseAttentionBlock(nn.Module):
    def __init__(
        ...
        **unused,
        ...
        # Define the Resisdual Weight for ReZero
        self.resweight = torch.nn.Parameter(torch.Tensor([0]), requires_grad=True) if is_rezero else 1
        # print('PairwiseAttentionBlock', p_layer_drop)
        self.layer_drop = DropPath(p_layer_drop) if p_layer_drop > 0. else nn.Identity()

        # if tri_new:
        #     self.triangle_multiply_outgoing = TriangleMultiplicativeModuleNew(dim=pair_embed_dim,
        #                                                                    is_use_ln=is_use_ln,
        #                                                                    mix='outgoing',
        #                                                                    is_sandwich_norm=is_sandwich_norm,
        #                                                                    pb_relax=pb_relax,
        #                                                                    )
        #     self.triangle_multiply_ingoing = TriangleMultiplicativeModuleNew(dim=pair_embed_dim,
        #                                                                   is_use_ln=is_use_ln,
        #                                                                   mix='ingoing',
        #                                                                   is_sandwich_norm=is_sandwich_norm,
        #                                                                    pb_relax=pb_relax,
    ...
    def forward(
        ...
        if exists(self.msa_embed_dim):
            x = self.msa2pair(msa_repr, x)
...
class DirectEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # print('DirectEncoderLayer', p_layer_drop)
        self.layer_drop = DropPath(p_layer_drop) if p_layer_drop > 0. else nn.Identity()
...
if __name__ == '__main__':
    ...
    # net = net.cuda().half()
    # pair_fea = pair_fea.cuda().half()
    # msa_fea = msa_fea.cuda().half()
