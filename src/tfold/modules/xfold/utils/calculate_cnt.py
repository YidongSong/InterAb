...
from tfold.modules.xfold.utils.utils import save_png
...
def eval_casp_from_dict(re_dict, CB_index=1, MODE = 'CASP13', val_root = '/mnt/superCeph2/private/user/shentao/DISTANCE_VAL', is_print = False):
...
if __name__ == '__main__':
    af_dist = f'/mnt/superCeph2/public/share/jonathanwu/for_scotttshen/CASP-Seq/fasta.files.casp14.domain.afolding/T1101-D2/distogram_model_1.npz'
    af_dist = dict(np.load(af_dist))
    af_dist = softmax_np(af_dist['logits'], axis=-1)
    dist = convert_distogram_af2trros(af_dist)
    save_png(logit_dist = dist, save_path='./tmp.png')
