...
As defined in `tfold/tools/prot_constants.py`, the maximal number of atoms per residue is `M = 14`,
and the maximal number of torsion angles per residue is `K = 7`, although the first two torsion
angles do not participate in the reconstruction of per-atom 3D coordinates.
...
When processed in `nn.Module`-based modules, an additional batch dimension will be added, which the
batch size fixed to `N = 1`.
