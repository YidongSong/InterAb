...
class PTmNet(nn.Module):
    ...
    def __init__(self, n_dims_pfea=256):
        ...
        self.n_bins_tmscore = 64

        # TMscore predictions
        self.net = nn.ModuleDict()
        self.net['tm'] = nn.Sequential(
            nn.Linear(self.n_dims_pfea, self.n_dims_pfea),
            nn.ReLU(),
            nn.Linear(self.n_dims_pfea, self.n_dims_pfea),
            nn.ReLU(),
            nn.Linear(self.n_dims_pfea, self.n_bins_tmscore),
        )

    def forward(self, pfea_tns, asym_id=None):
        ...
        * tmsc_dict: dict of pTM (and ipTM) predictions
        ...
        logits = self.net['tm'](pfea_tns)  # N x L x L x C
        ...
        if asym_id is not None:
            asym_id = asym_id.to(pfea_tns.device)
            iptm = compute_ptmscore(logits[0], asym_id=asym_id, interface=True, pairwise=True)
            tmsc_dict['iptm'] = iptm
