...
class AtomAttentionEncoder(nn.Module):
    ...
    def forward(self, atom_inputs, atom_mask=None):
        ...
        atompair_feat_cond = atompair_feat_cond + atom_repr_cond_row.unsqueeze(-2) + atom_repr_cond_col.unsqueeze(-3)
...
class AtomToTokenPooler(nn.Module):
    ...
    def forward(self, atom_feats, molecule_atom_lens):
        ...
        token_feats = summed / molecule_atom_lens.clamp(min=1).unsqueeze(-1) # wait
...
class InterAb(nn.Module):
    def __init__(self, heavy_dir,light_dir, antigen_dir, emb_dim=256):
        ...
        self.atom_attn_encoder = AtomAttentionEncoder(n_dims_atom_inputs=42, n_dims_atom=64, n_dims_atompair=16, atoms_per_window=512, n_dims_token=384, n_dims_sfea=384, n_dims_pfea=128, atom_transformer_blocks=2, atom_transformer_heads=4)
    ...
    def forward(self, heavy, light, antigen, data_dict, data_batch, device):
        # sequence-model
        heavy_encoded = self.HeavyModel(**heavy).last_hidden_state # batch 4, 2493/15360
        light_encoded = self.LightModel(**light).last_hidden_state # batch 4, 3029/15360
        antigen_encoded = self.AntigenModel(**antigen).last_hidden_state # batch 4, 9055/15360
        ...
        if len(atom_enc.shape) == 1:
            atom_enc = atom_enc.unsqueeze(0) # if batch_size == 1
        # atom_enc = self.predict(atom_enc)
        try:
            concated_encoded = torch.concat((heavy_cls,light_cls,antigen_cls, h_V_con, atom_enc) , dim = 1)
        except:
            pdb.set_trace()

        # concated_encoded = torch.concat((heavy_cls,light_cls,antigen_cls, h_V_con), dim = 1)
