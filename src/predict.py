...
def predict(model ,data_loader, antibody_tokenizer, antigen_tokenizer,device,config, task):
    if task == 'affinity':
        if not os.path.exists('./data/atom/exa_affinity_atom_inputs.pkl'):
            # Replace it with the path to your PDB file
            atom_input.get_atom_inputs(task, PDB_Path = './data/atom/complex_pdb/') 

        if not os.path.exists('./data/Geo_data/exa_affinity_geo_data.pkl'):
            # Replace the path in the file with your own path
            Geo_Data.get_geo_data(task)
    ...
    if task == 'specificity':
        if not os.path.exists('./data/atom/exa_specificity_atom_inputs.pkl'):
            # Replace it with the path to your PDB file
            atom_input.get_atom_inputs(task, PDB_Path = './data/atom/complex_pdb/')
        
        if not os.path.exists('./data/Geo_data/exa_specificity_geo_data.pkl'):
            # Replace the path in the file with your own path
            Geo_Data.get_geo_data(task)
