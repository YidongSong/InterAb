...
def geo_data(batch_name):
    ...
    with open("./data/Geo_data/exa_data.pkl", "rb") as f:
            dataset = pickle.load(f)
    # Replace with the precomputed ESM features
...
def get_geo_data(task):
    prepare_data_feat.prepare_geo_data()
    # Please replace with your data path
    data = pickle.load(open('data/Geo_data/exa_data.pkl', 'rb'))
    name = []
    for key in data.keys():
        name.append(key)

    data_dict = geo_data(name)
    pickle.dump(data_dict, open('./data/Geo_data/exa_{}_geo_data.pkl'.format(task), 'wb'))